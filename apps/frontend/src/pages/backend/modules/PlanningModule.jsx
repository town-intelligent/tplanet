...
import { SITE_HOSTERS, FAQ_ITEMS } from "../../../utils/Config";
...
export default function PlanningModule({ onConfirm, onSend }) {
  ...
  // 載入計劃列表 - 從 SITE_HOSTERS[1:] 所有 email 載入
  useEffect(() => {
    async function loadPlans() {
      setLoadingPlans(true);
      try {
        const emails = SITE_HOSTERS.slice(1); // 取得從索引 1 開始的所有 email
        const allPlans = [];
    ...
    loadPlans();
  }, []); // 不依賴 email，因為我們要載入所有 SITE_HOSTERS 的計劃
