...
import { departments as STATIC_DEPARTMENTS, sdgData, SITE_HOSTERS } from "../../../utils/Config.jsx";
...
const KPIList = () => {
  ...
  const fetchMultipleBudgetSummary = async (emails) => {
    ...
    try {
      const requests = emails.map((email) =>
        fetch(`${API_BASE}/dashboard/budget`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ email }),
        })
      ...
      // 初始化彙總表
      const baseMap = {};
      STATIC_DEPARTMENTS.forEach((name) => {
      ...
      // 生成下拉清單
      const seen = new Set(STATIC_DEPARTMENTS);
      const extrasOrdered = extraNames.filter((n) => {
        if (seen.has(n)) return false;
        seen.add(n);
        return true;
      });
      const options = [...STATIC_DEPARTMENTS, ...extrasOrdered];
