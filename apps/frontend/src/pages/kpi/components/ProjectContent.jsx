...
export default function ProjectContent() {
  ...
  useEffect(() => {
    const checkAttachment = async () => {
      ...
      setError(null);
      try {
        const response = await fetch('https://beta-tplanet-backend.ntsdgs.tw/projects/attachment_exist', {
          method: 'POST',
          headers: {
  ...
  const handleDownload = async () => {
    ...
    try {
      const response = await fetch('https://beta-tplanet-backend.ntsdgs.tw/projects/download_attachment', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  if (!project) return <div>首次載入可能需要較長時間，請耐心等待</div>;
