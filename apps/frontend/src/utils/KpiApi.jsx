...
const hoster = SITE_HOSTERS.length > 1 ? SITE_HOSTERS.slice(1) : SITE_HOSTERS[1];
 ...
 */
export async function fetchSdgsProjects(email = hoster, year = null) {
  // 構建 payload：有提供 year 才加進去
  const payload = { email };
  if (year) payload.year = year;

  const res = await fetch(`${API_BASE}/dashboard/sdgs`, {
    method: "POST",
