// src/utils/New.js
import { SITE_HOSTERS } from "./Config.jsx";
...
export async function news_list(email) {
  try {
    ...
    const res = await fetch(`${API_BASE}/news/news_list`, {
      method: "POST",
      body: form,
    });
...
export async function news_delete(uuid, email) {
  ...
  try {
    ...
    const res = await fetch(`${API_BASE}/news/news_delete`, {
      method: "POST", 
      body: form,
    });
...
export async function news_update(uuid, form) {
  try {
    ...
    const response = await fetch(
      `${API_BASE}/news/news_update?uuid=${encodeURIComponent(uuid)}`,
      {
...
export function isAuthorizedSiteHoster(email) {
  return SITE_HOSTERS.includes(email);
}
